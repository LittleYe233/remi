...
def item2event(groups):
    ...
    for i in range(len(groups)):
        ...
        for item in groups[i][1:-1]:
            # position
            flags = np.linspace(
                bar_st, bar_et, DEFAULT_FRACTION, endpoint=False)
            ...
            if item.name == 'Note':
                ...
                if tempo in DEFAULT_TEMPO_INTERVALS[0]:
                    tempo_style = Event(
                        'Tempo Class', item.start, 'slow', None)
                    ...
                    tempo_style = Event(
                        'Tempo Class', item.start, 'fast', None)
                    tempo_value = Event('Tempo Value', item.start,
...
def write_midi(words, word2event, output_path, prompt_path=None):
    ...
    for note in temp_notes:
        if note == 'Bar':
            ...
            flags = np.linspace(current_bar_st, current_bar_et,
                                DEFAULT_FRACTION, endpoint=False, dtype=int)
            st = flags[position]
            # duration (end time)
            et = st + duration
